...
작품 AGENTS와 현재 사용자 지시가 이 문서의 Grok 시안→Codex 정본 예시보다 우선한다. 단독 제작·내장 이미지 도구·특정 백엔드 중단 지시를 유지한다. 템플릿의 `generation`은 현재 선택한 기본값이며 모든 작품의 속도·비용·이용 가능성을 보장하지 않는다.

템플릿 `lettering_mode`는 기본 `embedded`, 후식자는 `post`다. `post`에서는 대사·화자 검증을 유지하고 원화 프롬프트에는 무문자와 식자 여백만 요청한다. 대사 원문은 관리 파일에 보존한다. 생성 직후 JPG는 **후식자 전 중간본**이며 OCR 누락 경고를 자동 통과시키지 않는다. 별도 렌더러로 고정 폰트·말풍선을 실제 JPG에 반영하고 최종본에 OCR·690/390px 시각 검수를 다시 수행한 뒤 채택·납품한다. 자동 식자 기능은 제공하지 않는다.

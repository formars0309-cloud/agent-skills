...
6. 참고 문서는 필요한 시점에만 연다. 한꺼번에 미리 읽지 않는다.
   - 1단계 시작 전: [실측·새 세션·교차 검수 규칙](references/cross-review-rules.md)
   - 3·6단계 검수를 띄우기 전: [감사·승인 규칙](references/review-and-approval-rules.md), [검수 실행기 사용법](references/audit-runner.md)
   - 운영 발행 직후: [발행 후 필수 마무리](references/post-publish.md)

## 필수 절차 요약 — 2026-09-07 사용자 지정

세부 원문: [실측·새 세션·교차 검수 규칙](references/cross-review-rules.md). **1단계 시작 전과 3·6단계 검수를 띄우기 전에 반드시 읽는다.**

- 1단계에서 `blueocean`으로 수요·공급·상업성·추세를 실측하고 명령·수치·채택 이유를 남긴다. 실측을 못 하면 2단계로 가지 않는다.
- 3·6단계 검수와 모든 재검수는 **매번 새 Orca 터미널의 새 세션**에서 한다. resume·서브에이전트 대체·작성 대화나 이전 판정 전달은 금지.
- 검수자는 **작성에 참여하지 않은 반대쪽 제품**이다: Codex 작성 → Claude Code 검수, Claude Code 작성 → Codex 검수. 반대쪽을 쓸 수 없으면 검수를 보류한다(자체 검수로 대체 금지).
- 검수 기록에는 실제 터미널·세션 ID, 모델·CLI 버전, 시각, 입력 SHA-256, 원출력 경로, 종료 코드를 남기고 회수 때 입력 SHA를 다시 대조한다.
...
3·6단계 검수는 스킬의 `scripts/run-audit.mjs`로만 띄운다(즉석 스크립트 금지). 명령·입력 폴더 구성·종료 코드(0 통과·2 수정 후 확인·3 반려·4 상한 소진·5 입력 실패·6 실행 보류)·대기 방법은 [검수 실행기 사용법](references/audit-runner.md)을 **실행기를 처음 부르기 전에** 읽는다.
...
운영 발행 뒤에는 **운영 URL 확인 → Google Search Console 색인 생성 요청·접수 확인 → `harness/publication/<slug>.md`와 `harness/PROGRESS.md` 기록 → 이 글의 작업 터미널 전체 종료(작성 터미널은 커밋·푸시 뒤 마지막 명령으로 자기 자신을 닫음)**까지 한다. 완료 기준은 운영 확인 통과 + 색인 요청 접수 + 해당 작업 터미널 0개다.

절차·예외·자기 종료 명령 원문: [발행 후 필수 마무리](references/post-publish.md). **발행 직후, 마무리를 시작하기 전에 읽는다.**

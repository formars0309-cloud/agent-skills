---
name: naver-account
description: 네이버 블로그 여러 개를 Aside로 다룰 때 블로그 ID마다 전용 브라우저 프로필(Aside 계정 u0/u1/u2)을 골라 쓰고, 입력·임시저장·발행 직전에 로그인 계정이 대상 블로그와 맞는지 확인한다. 로그인이 풀렸으면 올바른 프로필에 로그인 창을 준비한다. 네이버 블로그 업로드·발행·수정, "네이버 계정 확인", "어느 프로필로", "네이버 로그인 풀림"에 사용한다.
---
...
| codori_game | u1 | Profile 1 | `aside-codori-game` | 명조 관측소(게임) |
| demiandew | u2 | Profile 2 | `aside-demiandew` | 행복한 거북이 유입용 |
...
2. 계정을 확인한다.
   ```bash
   python3 ~/.claude/skills/naver-account/scripts/naver_account.py check <블로그ID>
   ```
...
4. 로그아웃(2)이면 로그인 창을 준비한다.
   ```bash
   python3 ~/.claude/skills/naver-account/scripts/naver_account.py login <블로그ID> --wait 600
   ```

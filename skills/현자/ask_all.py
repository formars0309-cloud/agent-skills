#!/usr/bin/env python
"""여러 AI CLI에 같은 질문을 동시에 던지고 답을 한 화면에 모은다.

사용:
  askall "질문"                      # 기본: claude codex grok gemini
  askall -a claude,grok "질문"       # 일부만
  askall -t 600 "질문"               # 타임아웃(초, 기본 300)
  askall --no-open "질문"            # 브라우저 안 열고 터미널 출력만
  echo "여러 줄 질문" | askall        # 질문을 stdin으로

결과는 ~/askall/<시각>.md 와 .html 로 저장되고, HTML이 기본 브라우저로 열린다.
표준 라이브러리만 쓴다. 각 CLI는 빈 임시 폴더에서 실행해 현재 프로젝트를 읽지 않게 한다.
"""
import argparse
import datetime
import html
...
def cmd_for(agent, q, tmpdir):
    ...
    return {
        ...
        'grok': ['grok', '-p', q],
        'gemini': ['gemini', '--skip-trust', '-p', q],  # 임시 폴더는 신뢰 목록에 없다
    }[agent], out


def ask(agent, q, timeout, tmpdir):
    ...
    try:
        p = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='replace',
                           timeout=timeout, cwd=tmpdir, stdin=subprocess.DEVNULL)
        text = ''
...
def main():
    ...
    tmp = tempfile.mkdtemp(prefix='askall-')
    print('질문:', q, file=sys.stderr)
    print('실행 중: %s (최대 %d초)' % (', '.join(agents), args.timeout), file=sys.stderr)
    with ThreadPoolExecutor(len(agents)) as ex:
        results = list(ex.map(lambda a: ask(a, q, args.timeout, tmp), agents))
    shutil.rmtree(tmp, ignore_errors=True)

    stamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
    outdir = pathlib.Path.home() / 'askall'
    ...
    cols = ''.join(
        ...
        for agent, text, sec in results)
    page = ('<!doctype html><meta charset="utf-8"><title>askall %s</title>'
